/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gtretiak <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/12 17:19:30 by gtretiak          #+#    #+#             */
/*   Updated: 2025/03/30 13:09:09 by gtretiak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int	ft_check_input(char *cmd, int code)
{
	if (code == 1)
	{
		if (!ft_strncmp(cmd, "rrb\n", 4) || !ft_strncmp(cmd, "rrr\n", 4)
			|| !ft_strncmp(cmd, "rra\n", 4)
			|| !ft_strncmp(cmd, "ra\n", 3)
			|| !ft_strncmp(cmd, "rb\n", 3)
			|| !ft_strncmp(cmd, "rr\n", 3)
			|| !ft_strncmp(cmd, "sa\n", 3)
			|| !ft_strncmp(cmd, "sb\n", 3)
			|| !ft_strncmp(cmd, "ss\n", 3)
			|| !ft_strncmp(cmd, "pa\n", 3)
			|| !ft_strncmp(cmd, "pb\n", 3))
			return (0);
		else
			return (1);
	}
	else if (code == 2)
		return (1);
	return (0);
}

char	*ft_fill_tmp(char *tmp, char *buf, int fd)
{
	ssize_t	x;
	char	*new_tmp;

	x = 1;
	while (x > 0)
	{
		x = read(fd, buf, BUFFER_SIZE);
		if (x == -1)
		{
			free(tmp);
			free(buf);
			return (NULL);
		}
		buf[x] = '\0';
		new_tmp = ft_strjoin(tmp, buf);
		free(tmp);
		tmp = new_tmp;
		if (x == 0)
			break ;
		if (ft_strchr(buf, '\n'))
			break ;
	}
	free(buf);
	return (tmp);
}

char	*ft_get_line(char *tmp)
{
	char	*res;
	int		i;

	i = 0;
	if (tmp[0] == '\0')
		return (NULL);
	while (tmp[i] && tmp[i] != '\n')
		i++;
	res = ft_substr(tmp, 0, i + 1);
	if (tmp[i] == '\n')
		i++;
	res[i] = '\0';
	return (res);
}

char	*ft_take_rest(char *tmp, int code)
{
	int		i;
	char	*new_tmp;

	if (code)
	{
		free(tmp);
		return (NULL);
	}
	i = 0;
	while (tmp[i] != '\n' && tmp[i])
		i++;
	if (tmp[i] == '\0')
	{
		free(tmp);
		return (NULL);
	}
	new_tmp = ft_substr(tmp, i + 1, ft_strlen(tmp));
	free(tmp);
	return (new_tmp);
}

char	*get_next_line(int fd, int check_input)
{
	static char	*tmp;
	char		*res;
	char		*buf;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (!tmp)
		tmp = ft_strdup("");
	buf = malloc(sizeof(char) * (BUFFER_SIZE + 1));
	if (!buf)
		return (NULL);
	tmp = ft_fill_tmp(tmp, buf, fd);
	if (!tmp)
	{
		free(tmp);
		tmp = NULL;
		return (NULL);
	}
	if (check_input)
		check_input = ft_check_input(tmp, check_input);
	res = ft_get_line(tmp);
	tmp = ft_take_rest(tmp, check_input);
	return (res);
}
/*
static int	ft_countlines(const char *s, char c);

static int	ft_putstr_fd(char *s, int fd);

int	main(void)
{
	int	fd;
	int	newfd;
	int	i;
	char	*s = NULL;
	char	*str = "Numbers:\nDec:0123456789\nHex:0123456789ABCDEF\nBin:01\0";
	size_t	written;
	int	words;
	
	fd = 0;
	i = 1;
	newfd = open("file.txt", O_RDWR | O_CREAT | O_TRUNC);
	if (newfd == -1)
		printf("Error with a file: {-1}...program should stop...\n");
	else
		fd = newfd;
	written = ft_putstr_fd(str, fd);
	words = ft_countlines(str, '\n');
	lseek(fd, 0, SEEK_SET);
	if (written != ft_strlen(str))
		printf("Error writing to file\n");
	while (words)
	{
		s = get_next_line(fd);
		if (s == NULL)
			printf("Get_next_line returned NULL\n");
		else
			printf("Line %d:%s", i++, s);
		words--;
		free(s);
	}
}

static int	ft_putstr_fd(char *s, int fd)
{
	int	i;

	i = 0;
        while (*s)
	{
                write(fd, s++, 1);
		i++;
	}
	return (i);
}

static int      ft_countlines(const char *s, char c)
{
        size_t  i;
        size_t  count;

        i = 0;
        count = 0;
	if (s[i] == c) 
		count++;
        while (s[i])
        {
                while (s[i] == c)
                        i++;
                if (s[i])
                        count++;
                while (s[i] && s[i] != c)
                        i++;
        }
        return (count);
}*/
