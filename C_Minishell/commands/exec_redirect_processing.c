...
/*   Updated: 2025/05/14 14:21:40 by gtretiak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	redirect_execution(t_token *cmd, t_mini *ms, int fd, int is_input)
{
	int	status;
	int	original_fd;
...
int	exec_redirect(t_token *token, t_mini *ms)
{
			...
			|| token->type == CMD_PIPE))
	{
		...
		fd_and_status[1] = redirect_execution(cmd, ms, fd_and_status[0], is_input);
